...
int nta_expand(string input_string, string& output_string, vector<int> keylist){
    ...
    if(temp_output_string.rfind("NTA~")!=string::npos){
        ...
        temp_output_string.erase(pos, to_delete.length());
        nta_report(3,"Found layer, switching to " + to_string(got_level));
        }
        catch(const invalid_argument) {
            nta_report(3,"No integer in NTA~, automatically reverting to default layer");
            got_level = 0;
        }
    }
...
public:
    ...
    void execute(vector<int> keylist, int &level)
    {
        ...
        if(new_level!=-1) {
            nta_report(2,"Changing level to " + to_string(new_level));
            level = new_level;
        }
    }
